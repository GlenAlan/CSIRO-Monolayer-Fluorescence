# -*- coding: utf-8 -*-
"""
Created on Tue Jul  9 14:24:09 2024
...
def move_to(mcm301obj, stage, pos, vel):
    #position in mm
    print("*** Moving to specific position ", pos, "mm, at speed ", vel)
    ...
    direction = 0  # direction: 0:counter-clockwise. 1:clockwise
    result = mcm301obj.set_velocity(stage, direction, vel)
    if result < 0:
        print("set_velocity failed")
    else:
        print("set_velocity:", vel)

    step = 0.2116667 # step size in um
    target_encoder = math.floor(pos/(step/1000))
    print("Target encoder set to ", target_encoder)
    result = mcm301obj.move_absolute(stage, int(target_encoder))
    if result < 0:
        print("move_absolute failed")
    else:
        print("move_absolute succeeded")
...
# ------------ Home Stage -------------- #
def home_stage(mcm301obj, stage):
    print("*** Homing ")

    stage_params_info = [0]
    result = mcm301obj.get_stage_params(stage, stage_params_info)
    if result < 0:
        print("get_stage_params failed")
    else:
        print("get_stage_params:", stage_params_info)
    ...
    state = 1
    result = mcm301obj.set_chan_enable_state(stage, state)
    if result < 0:
        print("set_chan_enable_state failed")
    else:
        print("set_chan_enable_state:", state)
    ...
    velocity = 50
    direction = 0  # direction: 0:counter-clockwise. 1:clockwise
    result = mcm301obj.set_velocity(stage, direction, velocity)
    if result < 0:
        print("set_velocity failed")
    else:
        print("set_velocity:", velocity)

    result = mcm301obj.home(stage)
    if result < 0:
        print("home failed")
    else:
        print("home succeeded")
...
def move(stage, pos, vel):
    ...
    try:
        ...
        move_to(mcm301obj, stage, pos, vel)
        print("--------------------------- Move finished-------------------------")
        mcm301obj.close()
        print("_____CLOSED STAGES_____")
