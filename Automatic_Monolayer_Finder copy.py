...
def add_image_to_canvas(canvas, image, center_coords):
    ...
    new_canvas_height = max(canvas.shape[0], y + img_height)
    new_canvas_width = max(canvas.shape[1], x + img_width)

    if new_canvas_height > canvas.shape[0] or new_canvas_width > canvas.shape[1]:
    ...
    canvas[y:y + img_cropped.shape[0], x:x + img_cropped.shape[1]] = img_cropped

    return canvas


def stitch_and_display_images(frame_queue, canvas_queue, fixed_size=(500, 500)):
    ...
    while True:
        ...
        canvas_queue.put(display_canvas)
